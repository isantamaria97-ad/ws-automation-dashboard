...
JIRA_JQL = 'project = WSC AND "QA Story Points[Number]" is not EMPTY ORDER BY updated DESC'
...
def fetch_jira_tickets() -> list:
    ...
    try:
        ...
        if str(e) == "JQL_EPIC_LINK_UNSUPPORTED":
            ...
            jql = 'project = WSC AND "QA Story Points[Number]" is not EMPTY ORDER BY updated DESC'
            raw = paginate_jira(jql, "summary,status,assignee,updated,priority,labels")
        else:
            raise
